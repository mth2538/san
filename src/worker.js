...
// Variables
let uuid = '30d21b15-8b26-4ea5-ba8f-904ef6074526'; // 仅 SSELV 协议使用
let sha224EncodePassword = 'abcf0a23e0d54a719543f3d24d797c0fe8adf926c98019c675655b7f'; // najorT 协议的密码对应 sha224 值
let systemSig = '1a2b3c4d5e6f7g8h9i0j'; // 仅 skcoswodahS 协议使用，可以是uuid
let pathSig = ''; // 仅 skcoswodahS 协议使用
let rootDomain = ''; // Ganti dengan domain utama kalian
let serviceName = getBeforeDot(rootDomain); // Ganti dengan nama workers kalian
let cfApiTokens = ''; // Ganti dengan Global API key kalian (https://dash.cloudflare.com/profile/api-tokens)
let apiEmail = ''; // Ganti dengan email yang kalian gunakan
let accountID = ''; // Ganti dengan Account ID kalian (https://dash.cloudflare.com -> Klik domain yang kalian gunakan)
let zoneID = ''; // Ganti dengan Zone ID kalian (https://dash.cloudflare.com -> Klik domain yang kalian gunakan)
let subPassword = ''; // 订阅预设的访问密码
let subpwd = ''; // 订阅用户输入的密码，这个值在翻页中还要用到
let defaultDocDomain = 'example.com';
		...
		for (let i = startIndex; i < startIndex + PROXY_PER_PAGE; i++) {
			...
			for (const port of PORTS) {
				...
				for (const protocol of PROTOCOLS) {
					// Special exceptions
					if (protocol === 'ss') {
						let ssPassword = 'none'; // skcoswodahS 协议，设置该密码无效
						uri.username = btoa(`none:${ssPassword}`);
						uri.searchParams.set(
							'plugin',
							`v2ray-plugin${port == 80 ? '' : ';tls'};mux=0;mode=websocket;path=/${proxyIP}-${proxyPort}/${systemSig};host=${hostName}`
						);
					} else {
						uri.username = uuid;
...
export default {
	async fetch(request, env, ctx) {
		try {
			uuid = env.UUID4 || uuid;
			sha224EncodePassword = env.SHA224PWD || sha224EncodePassword;
			systemSig = env.SSSIG || systemSig;
			subPassword = env.SUBPWD || subPassword;
			defaultDocDomain = env.DEFAULTDOCDOMAIN || defaultDocDomain;

			// cloudflare
			apiEmail = env.CFEMAIL || apiEmail;
			accountID = env.CFACCOUNTID || accountID;
			zoneID = env.CFZONEID || zoneID;
			cfApiTokens = env.CFAPITOKENS || cfApiTokens;

			rootDomain = env.ROOTDOMAIN || rootDomain;
			serviceName = env.SERVICENAME || serviceName;

			const upgradeHeader = request.headers.get('Upgrade');
			const url = new URL(request.url);
			subpwd = url.searchParams.get('pwd') || subpwd;

			if (subPassword && subpwd) {
				subPassword = encodeURIComponent(subPassword);
				subpwd = encodeURIComponent(subpwd);
			}

			// Gateway check
			if (cfApiTokens && apiEmail && accountID && zoneID) {
				isApiReady = true;
			}

			// Handle proxy client
			if (upgradeHeader === 'websocket') {
				const pathVal = url.pathname;
				const addrRegex = /^\/([^\/]+[:=-]\d+)(?:\/(\S+))?/;
				const addrMatch = pathVal.match(addrRegex);
				if (addrMatch) {
					proxyIP = addrMatch[1];
					pathSig = addrMatch[2] || ''; // 仅skcoswodahS使用
					return await websocketHandler(request);
				}
				// 支持多国家代码和多IP地址
				const regex = /^\/([^/]+)(?:\/(\S+))?/;
				const commaMatch = pathVal.match(regex);
				if (commaMatch) {
					let part = commaMatch[1];
					pathSig = commaMatch[2] || ''; // 仅skcoswodahS使用

					const proxyKeys = part.split(',');
					const proxyKey = proxyKeys[Math.floor(Math.random() * proxyKeys.length)];
					if (proxyKey.length !== 2) {
						// /192.168.1.1-443,192.168.1.2-443
						proxyIP = proxyKey;
						return await websocketHandler(request);
					}
					// /ID、/SG、/SG,US
					const kvProxy = await getKVProxyList();
					// for kvProxyList.json
					proxyIP = kvProxy[proxyKey.toUpperCase()][Math.floor(Math.random() * kvProxy[proxyKey.toUpperCase()].length)];
					return await websocketHandler(request);
				}
			}

			if (url.pathname.startsWith('/sub') && subpwd === subPassword) {
				...
				const targetParam = url.searchParams.get('ip');
				const target = targetParam && targetParam.trim() !== '' ? targetParam.split(':') : [];
				if (target.length === 0) {
					return await reverseProxy(request, defaultDocDomain);
				}
			...
			} else if (url.pathname.startsWith('/api/v1') && subpwd === subPassword) {
								...
								if (protocol == 'ss') {
									...
									uri.searchParams.set(
										...
										`v2ray-plugin${port == 80 ? '' : ';tls'};mux=0;mode=websocket;path=/${proxy.proxyIP}-${
											proxy.proxyPort
										}/${systemSig};host=${APP_DOMAIN}` // 添加skcoswodahS的访问权限
									);
								} else {
									uri.username = uuid;
								}
			...
			new WritableStream({
				async write(chunk, controller) {
					...
					if (protocol === reverse('najorT')) {
						protocolHeader = parseNajortHeader(chunk, sha224EncodePassword);
					} else if (protocol === reverse('SSELV')) {
						protocolHeader = parseSselvHeader(chunk, uuid);
					} else if (protocol === reverse('skcoswodahS')) {
						protocolHeader = parseSsHeader(chunk, pathSig);
					} else {
...
function parseSsHeader(ssBuffer, userSig) {
	if (systemSig !== userSig) {
		return {
			hasError: true,
			message: "You can't use the skcoswodahS",
		};
	}
...
function parseSselvHeader(buffer, userID) {
	...
	if (stringifyUUID(new Uint8Array(buffer.slice(1, 17))) === userID) {
		isValidUser = true;
	}
	if (!isValidUser) {
		return { hasError: true, message: 'invalid userID' };
	}
	...
	// 0x01--> ipv4  addressLength =4
	// 0x02--> domain name addressLength=addressBuffer[1]
	// 0x03--> ipv6  addressLength =16
	let addressLength = 0;
...
function parseNajortHeader(buffer, sha224Password) {
	...
	if (!sha224Regex.test(sha224Password)) {
		return {
			hasError: true,
			message: 'invalid sha224EncodePassword',
		};
	}

	let passwordArray = new Uint8Array(buffer.slice(0, 56));
	let passwordStr = new TextDecoder().decode(passwordArray);
	if (passwordStr !== sha224Password) {
		return {
			hasError: true,
...
class CloudflareApi {
	constructor() {
		this.bearer = `Bearer ${cfApiTokens}`;
		this.accountID = accountID;
		this.zoneID = zoneID;
		this.apiEmail = apiEmail;
		this.cfApiTokens = cfApiTokens;

		this.headers = {
			Authorization: this.bearer,
			'X-Auth-Email': this.apiEmail,
			'X-Auth-Key': this.cfApiTokens,
		};
	}
...
class Document {
	...
	buildCountryFlag() {
		...
		for (const flag of new Set(flagList)) {
			flagElement += `<a href="/sub?pwd=${subpwd}&cc=${flag}${
...
function getBeforeDot(str) {
	if (!str) return ''; // 空字符串或 null、undefined 都处理
